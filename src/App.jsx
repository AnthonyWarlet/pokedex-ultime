...
export default function App() {
  ...
  const [cardPrices, setCardPrices] = useState(() => {
    const saved = localStorage.getItem("pokemonCardPrices");
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const totalValue = caught.reduce((sum, id) => {
    const price = parseFloat(cardPrices[id] || 0);
  ...
  const groups = [...groupedMap.entries()]
    .map(([base, forms]) => {
      const baseForm =
        forms.find((p) => p.name === base) || forms[0];

      const extraForms = forms.filter(
        (p) => p.name !== base
      );
      ...
      if (filterType === "caught" && !someCaught) {
        return null;
      }

      if (filterType === "uncaught" && allCaught) {
        return null;
      }
  ...
  return (
    <div style={{ padding: 20, fontFamily: "sans-serif" }}>
      ...
      >
        ...
        >
          Valeur totale des cartes capturées
        </div>
      ...
      {loading ? (
        ...
        >
          {items.map((item) => {
            ...
            const baseCaught = caught.includes(
              baseForm.id
            );
            ...
            return (
              <React.Fragment key={base}>
                ...
                >
                  ...
                  <PokemonImg
                    src={
                      baseForm.sprites.front_default
                    }
                    id={baseForm.id}
                    alt={base}
                    style={imgStyle}
